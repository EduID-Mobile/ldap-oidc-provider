...
module.exports.config = {
  ...
  // ???
    ...
    // issuerUrl: "https://eduid.htwchur.ch"
    //

  // pointer generator to the login page
    interactionUrl: function interactionUrl(interaction) { // eslint-disable-line no-unused-vars
    // this => koa context;

    // a more realistic example would point us to a authentication url
    // e.g., https://my.auth.org/interaction/<UUID>
    // actually it needs to be set up correctly.
    //
    // The oidc.uuid allows us to refer back to this provider.
    // The provider needs to have an endpoint that allows this script to
    // receive a notification when the authentication completed successfully.
    // The notification should be in the
    //    router.post('/interaction/:grant/confirm' ... handler).
        return `https://xyz.ch/interaction/${this.oidc.uuid}`;
    // a response could point to https://myserver.ch/interaction/uuid/userhash
    },
};

// the certificates (aka enc keys) should be served from the data store
// the keys here are all private keys!
...
// the integrityKeys (aka 'sign keys') should be served from the data store.
