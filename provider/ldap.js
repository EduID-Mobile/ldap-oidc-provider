...
const config   = settings.directoryOrganisation;
...
class LDAPAdapter {
    ...
    connect(userdn = null, passwd = null) {
        ...
        if (userdn === null) {
            userdn = this.opts.serviceDN;
            passwd = this.opts.servicePassword;
        }
    ...
    findPersonByUid(accountId) {
        let accountField = config.Account.id || "uid";
        let accountFilter = ["&", [`objectClass=${config.Account.class}`], [`${accountField}=${accountId}`]];

        return this.find(accountFilter, this.opts.userDN)
    ...
    findAndBind(filter, password) {
        ...
        return this.find(filter, this.opts.baseDN)
            .then(r => r.length ? r[0] : null)
            .then(e => {
                if (e) {
                    opt.baseDN = e.dn;
    ...
    loginAndBind(login, password) {
        let accountField = config.Account.bind || "mail";
        let accountFilter = ["&", [`objectClass=${config.Account.class}`], [`${accountField}=${login}`]];

        return this.findAndBind(accountFilter, password);
    }
